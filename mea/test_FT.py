...
if __name__ == "__main__":
    ...
    if is_G_loc:
        ...
        loading_G_iwn = np.genfromtxt("Green_loc_1D_U_10.000000_beta_20.000000_n_0.500000_N_tau_1024_Nit_20.dat",dtype=float,skip_header=1,usecols=(0,1,2))
        beta = 20.0 # Change this according to the parameters loaded
        ...
        plt.plot(wn_array,cubic_Spl_G_iwn_imag,marker='.',c="brown",label="cubic spline")
        plt.xlabel(r"$i\omega_n$")
        ...
        with open("test_bb.dat", "w") as f2:
            for i in range(len(K_iqn)):
                if i==0:
                    f2.write("/\n")
                if cs.isclose(K_iqn[i].real,0.0,abs_tol=1e-6): # To remove the tail that spoils the results, i.e 0.0 values
                    break
        ...
        for i,k in enumerate(k_array):
            ...
            if np.mod(i,10)==0:
                plt.plot(-G_tau_for_k[i,::-1],marker='o')
        ...
        plt.figure(1)
        for n in range(0,N_k,N_k//10):
            plt.plot( beta_array_self, G_tau_for_k[n,:], marker='o')

        iqn_array = []
        for i,k in enumerate(k_array):
            GG_tau_for_k = np.empty((len(beta_array_self),),dtype=float)
            for j in range(len(beta_array_self)):
                GG_tau_for_k[j] = velocity_squared(k)*(-2.0)*(-1.0)*G_tau_for_k[i,j]*G_tau_for_k[i,-1-j] # Eventually put the velocities as prefactors 
            ...
            der_GG_self = cs.Cubic_spline.get_derivative(beta_array_self,GG_tau_for_k)
            ...
            cubic_Spl_bb_iqn,*_ = cs.FermionsvsBosons.bosonic_corr_funct()
            bubble_GG_k_iqn_cubic[i,:] = cubic_Spl_bb_iqn
            # if np.mod(i,10)==0:
            #     plt.figure(2)
            #     plt.plot(cubic_Spl_bb_iqn)
            ####
            del GG_tau_for_k
        ...
        with open("test_bb_self.dat", "w") as f1:
