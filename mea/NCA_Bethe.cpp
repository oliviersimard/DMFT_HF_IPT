        ...
        for (double U=U_min; U<=U_max; U+=U_step){
            ...
            while (!is_converged && iter<MAXITER){
                ...
                for (size_t i=0; i<2*NTAU; i++){
                    ...
                    // AA
                    funct_k_integration_up = [&](double e,std::complex<double> iwn){
                        return DOS(e)/( iwn - h + U/2.0 - SE_iwn.slice(i)(up,up) - e*e / ( iwn + h + U/2.0 - SE_iwn.slice(i)(down,down) ) );
                    };
                    Gloc_iwn.slice(i)(up,up) = I1D(funct_k_integration_up,-e_max,e_max,iwn,"simpson");
                    // BB
                    funct_k_integration_down = [&](double e,std::complex<double> iwn){
                        return DOS(e)/( iwn + h + U/2.0 - SE_iwn.slice(i)(down,down) - e*e / ( iwn - h + U/2.0 - SE_iwn.slice(i)(up,up) ) );
                    };
                    Gloc_iwn.slice(i)(down,down) = I1D(funct_k_integration_down,-e_max,e_max,iwn,"simpson");
                ...
                std::string directory_container = "Bethe_lattice_U_"+std::to_string(U)+"_beta_"+std::to_string(beta)+"_n_"+std::to_string(0.5)+"_Ntau_"+std::to_string(NTAU);
                std::string full_path = "./data_Bethe_test_NCA_damping_0.05/"+directory_container;
                if (stat(full_path.c_str(), &info) == -1){
                    mkdir(full_path.c_str(), 0700);
                }
                // Saving data throughout iterations
                std::ofstream outSE(full_path+"/SE_NCA_AFM_U_"+std::to_string(U)+"_beta_"+std::to_string(beta)+"_N_tau_"+std::to_string(NTAU)+"_h_"+std::to_string(h)+"_Nit_"+std::to_string(iter)+".dat",std::ios::out);
                for (size_t i=0; i<2*NTAU; i++){
                    if (i==0){
