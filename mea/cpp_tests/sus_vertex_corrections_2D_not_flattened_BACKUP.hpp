    ...
    class OneLadder{
        ...
        */
        public:
            ...
            // explicit OneLadder(const SplineInline< T >& splInlineobj, const std::vector< T >& iqn, const std::vector<double>& k_arr, const std::vector<double>& twoD_gauss_quad, 
            //             const std::vector< T >& iqn_tilde, double mu, double U, double beta) : _splInlineobj(splInlineobj), _iqn(iqn), _k_t_b(k_arr), 
            //             _2D_gauss_quad(twoD_gauss_quad), _iqn_tilde(iqn_tilde){
            //     this->_mu = mu;
            //     this->_U = U;
            //     this->_beta = beta;
            // };
        ...
        private:
            ...
            // const std::vector<double>& _2D_gauss_quad;
            #endif
            const std::vector<double>& _sine_table;
            const std::vector<double>& _cosine_table;
            ...
            for (size_t n_tilde=0; n_tilde<NI; n_tilde++){
                ...
                inner_2D_int_jj = [&](double k_bar_x, double k_bar_y){
                    ...
                    return -1.0*(2.0*_sine_table[n_k_tilde_x])*(2.0*std::sin(k_bar_x)
                        ...
                        )*( 1.0/( _splInlineobj._iwn_array[n_bar] + _mu + 2.0*(std::cos(k_bar_x)+std::cos(k_bar_y)) - _splInlineobj.calculateSpline( (_splInlineobj._iwn_array[n_bar]).imag() ) )
