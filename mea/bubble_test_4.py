...
if __name__ == "__main__":
    ...
    bubble_GG_iqn_imag = np.asarray(list(map(lambda x: x.imag,bubble_GG_iqn)))
    ...
    with open("bubble_iqn_1.dat","w") as f:
        for i,qn in enumerate(qn_array):
            if i==0:
                f.write("/\n")
            if isclose(bubble_GG_iqn[i].real,0.0,abs_tol=1e-6):
                break
            f.write("%f\t\t%f\t\t%f\n" % (qn,bubble_GG_iqn[i].real,bubble_GG_iqn[i].imag)) # Setting im part to 0 to see...
    f.close()