...
// https://www.ucg.ac.me/skladiste/blog_10701/objava_23569/fajlovi/lu.pdf
...
void spline::iwn_tau_spl_extrm(){
    ...
    _Sp_1_0=m_c[0]; // f'_1(x_0) = 3a_1(x_0-x_1)^2 + 2b_1(x_0-x_1)^1 + c_1 = c_0
    ...
    _S_N_beta=m_a.back()*h_last*h_last*h_last+m_b.back()*h_last*h_last+m_c.back()*h_last+m_y.back();
    _Sp_N_beta=3.0*m_a.back()*h_last*h_last+2.0*m_b.back()*h_last+m_c.back();
    _Spp_N_beta=6.0*m_a.back()*h_last+2.0*m_b.back();
    // This will be used later for IFFT.
    for (size_t n=0; n<m_x.size(); n++){
