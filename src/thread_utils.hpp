...
template<typename T> 
inline void calculateSusceptibilitiesParallel(IPT2::SplineInline< std::complex<double> >,std::string,std::string,bool,bool);
...
namespace ThreadFunctor{
    ...
    class ThreadWrapper{
        public:
            ...
            explicit ThreadWrapper(HF::FunctorBuildGk Gk,HF::K_2D q,double ndo_converged);
            explicit ThreadWrapper(HF::K_1D q,IPT2::SplineInline< std::complex<double> > splInline);
            ThreadWrapper()=default;
            void operator()(size_t ktilde, size_t kbar, double b, bool is_jj); // 1D HF
            void operator()(size_t ktilde, size_t kbar, bool is_jj); // 1D IPT
            void operator()(size_t kbarx_m_tildex, size_t kbary_m_tildey); // 2D HF
            std::complex<double> gamma_oneD_spsp(double ktilde,std::complex<double> wtilde,double kbar,std::complex<double> wbar);
            std::complex<double> gamma_oneD_spsp_IPT(double ktilde,std::complex<double> wtilde,double kbar,std::complex<double> wbar);
            ...
            std::vector< std::complex<double> > buildGK1D_IPT(std::complex<double> ik, double k);
...
template<>
inline void calculateSusceptibilitiesParallel<IPT2::DMFTproc>(IPT2::SplineInline< std::complex<double> > splInline,std::string pathToDir,std::string customDirName,bool is_full,bool is_jj){
    std::ofstream outputChispspGamma, outputChispspWeights, outputChispspTotSus;// outputChispspBubbleCorr;
    std::string trailingStr = is_full ? "_full" : "";
    std::string frontStr = is_jj ? "jj_" : "";
        ...
        if (totSize % NUM_THREADS != 0){
            if ( (totSize-it)<NUM_THREADS ){
                ...
                for (size_t l=0; l<last_it; l++){
                    ...
                    lkt = static_cast<size_t>(floor(ltot/vecK.size())); // Samples the rows
                    lkb = (ltot % vecK.size()); // Samples the columns
                    std::thread t(std::ref(threadObj),lkt,lkb,is_jj);
                    tt[l]=std::move(t);
                    // tt[l]=thread(threadObj,lkt,lkb,beta);
                }
                threadObj.join_all(tt);
